...
export const blogPosts: BlogPost[] = [
  make("sendafrica-api-v2-sms-billing-changes", "API v2: what changed in SMS billing and idempotency", "Credits, parts, and idempotency keys — how SendAfrica charges exactly and how retries stay safe.", "September 21, 2026", "SMS API", "/images/blog/sendafrica-api-v2-sms-billing-changes.webp", "7 min read", [
  ...
  make("building-tanzania-first-sms-infrastructure", "Building SMS infrastructure", "How SendAfrica normalizes Tanzanian mobile numbers and recovers campaigns after worker interruptions.", "September 14, 2026", "Infrastructure", "/images/blog/building-tanzania-first-sms-infrastructure.webp", "8 min read", [
  ...
  make("mobile-money-top-ups-snippe-webhooks", "Mobile money top-ups: how Snippe webhooks grant credits", "From USSD prompt to signed callback to a posted credit balance.", "September 7, 2026", "Payments", "/images/blog/mobile-money-top-ups-snippe-webhooks.webp", "6 min read", [
  ...
  make("sender-id-registration-what-you-need", "Sender ID registration: documents and rules", "How to check the live requirements and submit a branded Sender ID request.", "August 28, 2026", "Sender IDs", "/images/blog/sender-id-registration-what-you-need.webp", "5 min read", [
  ...
  make("gsm-7-vs-ucs-2-why-your-message-split", "GSM-7 vs UCS-2: why your message split into parts", "Encoding and concatenation headers change the part count behind SMS credit usage.", "August 19, 2026", "SMS billing", "/images/blog/gsm-7-vs-ucs-2-why-your-message-split.webp", "6 min read", [
  ...
  make("api-key-rotation-and-security", "API key rotation and security best practices", "Store SA-prefixed API keys safely, rotate them cleanly, and keep secrets out of client code.", "August 11, 2026", "Security", "/images/blog/api-key-rotation-and-security.webp", "5 min read", [
  ...
  make("campaign-worker-crash-resume", "Crash-resume: how the campaign worker avoids duplicate recipients", "Redis locks, durable message records, and recovery when a campaign worker stops.", "August 3, 2026", "Campaigns", "/images/blog/campaign-worker-crash-resume.webp", "8 min read", [
  ...
  make("credit-ledger-atomic-idempotent", "The credit ledger: atomic, idempotent, never stale", "How SendAfrica records credit mutations and keeps balance reads current.", "July 26, 2026", "Credits", "/images/blog/credit-ledger-atomic-idempotent.webp", "7 min read", [
  ...
  make("payg-vouchers-tiered-tariffs", "Pay-as-you-go vouchers and tiered TZS tariffs", "Use the live rate endpoint to explain custom amount top-ups and credit estimates.", "July 18, 2026", "Payments", "/images/blog/payg-vouchers-tiered-tariffs.webp", "6 min read", [
  ...
  make("send-your-first-sms-with-curl", "Send your first SMS with curl", "A safe first request using the SendAfrica REST API and an environment-based key.", "July 9, 2026", "Getting started", "/images/blog/send-your-first-sms-with-curl.webp", "5 min read", [
  ...
  make("bulk-sms-api-partial-results", "Bulk SMS API: reading per-recipient results", "One request can target up to 1,000 recipients and report outcomes for each number.", "June 30, 2026", "Bulk SMS", "/images/blog/bulk-sms-api-partial-results.webp", "7 min read", [
  ...
  make("bulk-sms-or-campaign", "Bulk SMS API or campaign: which should you use?", "Pick a synchronous batch for a bounded send and a campaign for larger, tracked delivery work.", "June 22, 2026", "Bulk SMS", "/images/blog/bulk-sms-or-campaign.webp", "6 min read", [
  ...
  make("python-sdk-send-sms", "Send SMS with the Python SDK", "Initialize SendAfrica once, send from a server-side app, and inspect the result.", "June 15, 2026", "Python SDK", "/images/blog/python-sdk-send-sms.webp", "5 min read", [
  ...
  make("typescript-sdk-bulk-sms", "Bulk SMS with the TypeScript SDK", "Use the typed bulk resource and handle recipient-level outcomes in Node.js.", "June 6, 2026", "TypeScript SDK", "/images/blog/typescript-sdk-bulk-sms.webp", "6 min read", [
  ...
  make("go-sdk-retries-and-idempotency", "Go SDK retries and idempotency keys", "Keep transport retries from becoming duplicate business actions in Go.", "May 28, 2026", "Go SDK", "/images/blog/go-sdk-retries-and-idempotency.webp", "7 min read", [
  ...
  make("sendafrica-cli-first-send", "SendAfrica CLI: from health check to first send", "Install the CLI, configure credentials safely, and verify API access before sending.", "May 19, 2026", "CLI", "/images/blog/sendafrica-cli-first-send.webp", "6 min read", [
  ...
  make("api-keys-vs-jwt", "API keys vs JWTs: choosing the right SendAfrica credential", "Understand server-to-server keys and dashboard session tokens without mixing their roles.", "May 11, 2026", "Authentication", "/images/blog/api-keys-vs-jwt.webp", "6 min read", [
  ...
  make("webhook-delivery-status", "Delivery webhooks: update message state safely", "Verify callbacks, handle duplicates, and distinguish accepted SMS from delivery reports.", "May 2, 2026", "Webhooks", "/images/blog/webhook-delivery-status.webp", "7 min read", [
  ...
  make("idempotency-key-design", "Designing idempotency keys for SMS workflows", "Choose stable operation identifiers so safe retries replay the intended result.", "April 24, 2026", "Reliability", "/images/blog/idempotency-key-design.webp", "6 min read", [
  ...
  make("contact-lists-and-csv-import", "Contact lists and CSV import for SMS", "Prepare a clean contact list and understand how imported Tanzania numbers are handled.", "April 16, 2026", "Contacts", "/images/blog/contact-lists-and-csv-import.webp", "6 min read", [
  ...
  make("campaign-scheduling-and-time", "Campaign scheduling and time zones", "Create a scheduled campaign with an explicit time and verify when the worker will pick it up.", "April 7, 2026", "Campaigns", "/images/blog/campaign-scheduling-and-time.webp", "6 min read", [
  ...
  make("sms-api-error-handling", "SMS API errors: auth, credits, validation, and rate limits", "Build useful recovery paths for common SendAfrica API failures.", "March 30, 2026", "SMS API", "/images/blog/sms-api-error-handling.webp", "7 min read", [
  ...
  make("international-sms-rates", "Sending SMS beyond Tanzania: check the live rate card", "Use supported destination data rather than assuming a fixed African SMS price.", "March 21, 2026", "International SMS", "/images/blog/international-sms-rates.webp", "5 min read", [
  ...
  make("sender-id-usable-vs-registered", "Registered Sender IDs vs usable Sender IDs", "Why a pending registration request is not automatically a sender you can use.", "March 12, 2026", "Sender IDs", "/images/blog/sender-id-usable-vs-registered.webp", "4 min read", [
  ...
  make("message-logs-and-delivery-debugging", "Debugging delivery with message logs", "Trace an SMS using message IDs, status, recipient, credits, and request identifiers.", "March 3, 2026", "Observability", "/images/blog/message-logs-and-delivery-debugging.webp", "6 min read", [
  ...
  make("sms-parts-local-estimates", "Why SDK SMS part estimates can differ", "GSM-7 extension characters, Unicode code units, and normalization all affect exact billing.", "February 22, 2026", "SMS billing", "/images/blog/sms-parts-local-estimates.webp", "6 min read", [
  ...
  make("snippe-webhook-signature-verification", "Verify Snippe webhook signatures in Go", "Validate the timestamped HMAC against the exact request bytes before processing a payment event.", "February 13, 2026", "Webhooks", "/images/blog/snippe-webhook-signature-verification.webp", "7 min read", [
  ...
  make("rate-limits-and-retry-after", "Rate limits and Retry-After for SMS integrations", "Respect account plan limits and pace sends instead of flooding the API.", "February 4, 2026", "Reliability", "/images/blog/rate-limits-and-retry-after.webp", "5 min read", [
  ...
  make("campaign-partial-failure-and-credits", "Campaign partial failures and credit refunds", "Read recipient results and understand how failed SMS attempts affect credit accounting.", "January 26, 2026", "Campaigns", "/images/blog/campaign-partial-failure-and-credits.webp", "7 min read", [
  ...
  make("bulk-sms-provider-gateways", "How provider capabilities shape bulk SMS", "SendAfrica uses provider bulk endpoints when available and falls back to recipient sends when needed.", "January 17, 2026", "Bulk SMS", "/images/blog/bulk-sms-provider-gateways.webp", "6 min read", [
